...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        const role = message.mentions.roles.first(1)[2] || message.guild.roles.cache.find(role => role.id === `${args[1]}`) || message.guild.roles.cache.find(role => role.name === `${args.slice(1).join(" ")}`)
